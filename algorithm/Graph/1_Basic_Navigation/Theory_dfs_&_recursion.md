...
# ✅ 함수 내 if문의 리턴과 함수내의 리턴
- "union-find-basic.py"에서 확인 가능
> `if`문 안에서 `return`한 값은  
> **그 함수를 호출한 쪽으로 그대로 전달**되고,  
> 함수가 끝나면 그 값이 **밖으로 튕겨 나와서 반환**되는 거야.
...
### 만약 parents = [0, 0, 1] 이고  
`find_set(2)`를 호출했다고 해보자.

- `find_set(2)` → parents[2] = 1 → 대표자 아님  
- `find_set(1)` → parents[1] = 0 → 대표자 아님  
- `find_set(0)` → parents[0] = 0 → 대표자! → `return 0`

이 `0`은 **계속 위로 return되어 전달돼**:
...
즉, `find_set(2)`의 최종 결과는 `0`이 되는 거야.

---

## 간단하게 비유하면

재귀 함수는 마치 값을 들고 계단을 내려갔다가,  
**찾은 값을 들고 다시 계단을 올라오는** 것처럼” 작동해!

---

## 결론 한 줄 요약

> `if`문에서 리턴한 값은 함수 전체의 리턴값이 되고,  
> 그 값은 호출한 쪽으로 그대로 전달돼!  
> 그래서 **재귀 함수에선 return을 꼭 붙여야 흐름이 이어지는 거야.**
