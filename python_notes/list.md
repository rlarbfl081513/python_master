...
---
# DAT
- 데이터를 빠르게 조회하거나 업데이트하기 위해 값 자체를 인덱스로 사용하는 배열을 말함
- 카운팅이나 존재여부 체크등에 사용됨

1. 예시 코드
    - [DAT 사용하기 예시](../algorithm/List_&_String/List/list_2/samsung-bus.py)
   - 정류장 번호를 배열의 인덱스로 사용
...
```
<map object at 0x0000024a2eaff4c0>
```

이건 파이썬에서 **`map()` 함수가 반환하는 객체의 기본 출력 형식**인데, 그 이유와 성질을 하나씩 정리해줄게.
...
* `map()`은 결과를 **즉시 계산하지 않고**, **필요할 때 값을 하나씩 꺼내 쓰는** **이터레이터(iterator)** 를 반환해.
* 그래서 그냥 `print(map(...))` 하면 리스트처럼 보이지 않고 이런 **객체 정보**만 나오는 거야:
...
> **`map()`은 지연 평가(lazy evaluation)를 하기 때문**이야.

* 즉, **`list(map(...))`처럼 강제로 꺼내서 봐야** 실제 결과가 보임.
* 이런 특성은 **메모리 절약**과 **속도 향상**에 유리하지만, 그냥 출력하려고 하면 위처럼 정체불명의 객체처럼 보임.
...
```python
print(list(map(len, arr.split('0'))))
```

또는

```python
print(max(map(len, arr.split('0'))))  # 이렇게 쓰면 바로 정답 출력됨
```
...
| 함수                 | 설명                       |
| ------------------ | ------------------------ |
| `map()`            | 지연 평가. 함수 적용한 결과를 하나씩 꺼냄 |
| `filter()`         | 조건에 맞는 요소만 지연 평가로 반환     |
| `range()` (in Py3) | 예전엔 리스트였지만 지금은 지연 평가 객체  |
| `zip()`            | 묶은 튜플을 하나씩 지연 평가로 반환     |

---

## ✅ 정리

> `map()`은 **리스트가 아니라 이터레이터라서** 그냥 출력하면 `<map object at 0x...>` 이렇게 보이고,
> 실제 값을 보고 싶으면 `list()`나 `max()` 같은 함수로 꺼내야 해.
